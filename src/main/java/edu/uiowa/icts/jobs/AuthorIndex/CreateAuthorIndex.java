...
public class CreateAuthorIndex extends Configured implements Tool {
	  private static final Logger LOG = Logger.getLogger(CreateAuthorIndex.class);
	...
	public static void main(String[] args)  {
	    try {
			ToolRunner.run(new Configuration(), new CreateAuthorIndex(), args);
		} catch (Exception e) {
    ...
    public int run(String[] args) throws Exception
    {
       ...
       String outPutColumnName = "Author";
       if (options.has("o")) {
       ...
       LOG.info("Starting Job");
		try {
			...
			Job job = new Job(getConf(), "CreateAuthorIndex");
            job.setInputFormatClass(ColumnFamilyInputFormat.class);
		    job.setNumReduceTasks(0);

            job.setJarByClass(CreateAuthorIndex.class);
